...
OUTPUT_ROOT="$PROJECT_ROOT/experiments/vp/phase1_runtime"
...
if [[ -n "$before_status" || -n "$before_diff" ]]; then
    {
        echo "Phase 1 aborted: upstream is not clean before execution."
        echo "Integrity record: $BEFORE_PATH"
    } | tee "$LOG_PATH"
    {
        echo "baseline_exit_code=not_started"
        echo "final_exit_code=2"
...
if [[ ! -x "$PYTHON_BIN" ]]; then
    echo "Phase 1 aborted: Python executable not found: $PYTHON_BIN" | tee "$LOG_PATH"
    {
        echo "baseline_exit_code=not_started"
        echo "final_exit_code=2"
        echo "elapsed_seconds=0"
...
export PHASE1_MODEL_ROOT="$MODEL_ROOT"
export PHASE1_RESULT_ROOT="$RESULT_ROOT"
...
entry.cfg.models_dir = os.environ["PHASE1_MODEL_ROOT"]
entry.cfg.results_dir = os.environ["PHASE1_RESULT_ROOT"]
...
def audited_create_dataset(*args, **kwargs):
    ...
    print(f"PHASE1_DATASET_NAME={dataset_name}")
    print(f"PHASE1_DATASET_ROOT={entry.cfg.dataset[dataset_name]}")
    print(f"PHASE1_DATASET_INCLUDE={include}")
    print(f"PHASE1_DATASET_SPLIT_LENGTHS={[len(dataset) for dataset in datasets]}")
    return datasets


def audited_record(self, prediction, ground_truth, videos, users, timesteps):
    if not audit_state["shape_printed"]:
        print(f"PHASE1_FIRST_BATCH_PREDICTION_SHAPE={tuple(prediction.shape)}")
        print(f"PHASE1_FIRST_BATCH_GROUND_TRUTH_SHAPE={tuple(ground_truth.shape)}")
        print(f"PHASE1_FIRST_BATCH_VIDEO_SHAPE={tuple(videos.shape)}")
...
print(f"PHASE1_SOURCE_ROOT=/workspace/NetLLM-source")
print(f"PHASE1_MODEL_ROOT={entry.cfg.models_dir}")
print(f"PHASE1_RESULT_ROOT={entry.cfg.results_dir}")
print(f"PHASE1_ARGS={args}")
entry.run(args)
PY
...
if [[ -f "$SUMMARY_PATH" ]]; then
    ...
    IFS=',' read -r total_video total_user total_mae total_rmse <<< "$total_row"
fi
...
echo "PHASE1_BASELINE_EXIT_CODE=$baseline_status"
echo "PHASE1_FINAL_EXIT_CODE=$final_status"
echo "PHASE1_ELAPSED_SECONDS=$elapsed_seconds"
echo "PHASE1_TOTAL_MAE=$total_mae"
echo "PHASE1_TOTAL_RMSE=$total_rmse"
echo "PHASE1_SUMMARY_PATH=$SUMMARY_PATH"
echo "PHASE1_DETAIL_PATH=$DETAIL_PATH"
echo "PHASE1_UPSTREAM_CLEAN_AFTER=$integrity_clean"
echo "PHASE1_STATUS_PATH=$STATUS_PATH"

if [[ "$integrity_clean" != "yes" ]]; then
    echo "Phase 1 integrity violation detected. No cleanup or reset was attempted."
fi
