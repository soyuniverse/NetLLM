...
docs/
  final/                           cross-phase conclusions, final manuals
    ...
    PAPER_ANALYSIS_CANDIDATES.md   2026-08-23: every analysis result to
                                    ...
                                    full-scale. Row #11 (2026-08-23, same
                                    day): the adaptive-K negative result --
                                    strength rated separately since it
                                    proves two independent things at once
                                    (the intervention design is valid;
                                    the velocity-based trigger is not).
    PRESENTATION_SCRIPT_20260823.md  actual spoken-word script (Korean,
                                    ~1,570 chars / 10 minutes) following
                                    results/presentation_20260816/
                                    presentation_storyline.md's slide
                                    order -- per-slide narration + figure
                                    filename + 1-2 numbers to emphasize,
                                    plus a 5-question anticipated-Q&A table
                                    with source-document citations
  ...
  experiment_phase/                phase-specific evidence, one dir per phase
    ...
    analysis/                      TAIL_ANALYSIS.md: which samples degrade
                                    ...
                                    2026-08-23 (same day) follow-up
                                    "후속 진단" section: the false-
                                    positive root cause traced further --
                                    5 history-derived features (velocity,
                                    its std/CV, acceleration, direction-
                                    reversal count) compared between the
                                    63 true- and 382 false-positive
                                    widened samples, none separate them
                                    (AUC 0.488-0.555, all p>0.15) --
                                    the 10-step history itself doesn't
                                    carry the signal needed, not a
                                    threshold-tuning problem. Also:
                                    reflected as a presentation slide
                                    (`results/presentation_20260816/
                                    presentation_storyline.md` slide 5) and
                                    a `PAPER_ANALYSIS_CANDIDATES.md` row
                                    (#11) -- narrative/paper value judged
                                    by whether a real hypothesis was
...
results/speculative/<timestamp>/   run_speculative_benchmark.py output
  ...
  consolidated/                    merged tables + all 7 figures: the 3
                                    ...
                                    ADAPTIVE_K_RESULTS.md above. Same-day
                                    follow-up (11th figure):
                                    adaptive_k_fp_diagnosis_boxplot.png +
...
results/presentation_20260816/     module-by-module slide package (2026-08-23
                                    ...
                                    2026-08-23 (same day) addendum:
                                    adaptive_k_negative_result.png
                                    (2-panel: overall-vs-degraded-group MAE
                                    direction contrast, true/false-positive
                                    composition of the 445 widened
                                    samples), generated by
                                    build_adaptive_k_negative_result_figure.py;
                                    presentation_storyline.md's slide 5
                                    extended with the adaptive-K
                                    hypothesis/test/refute/interpret
                                    narrative and time budget rebalanced
                                    (조합 2.0->1.5분, 심층분석 2.0->2.5분).
                                    docs/final/PRESENTATION_SCRIPT_20260823.md
                                    is the companion spoken script (not in
                                    this directory -- see docs/final/
                                    entry below).
