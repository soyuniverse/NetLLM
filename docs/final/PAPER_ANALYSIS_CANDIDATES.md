...
| 11 | **Adaptive-K negative result — 설계 검증 + 트리거 반증** | 항목4의 모집단 음의 상관(rho=-0.40) 예측을 실제 개입 실험으로 검증: 목표군(열화 상위 5%)에서는 설계대로 개선되지만, 그 개입을 트리거하는 평균-속도 임계값의 낮은 정밀도 때문에 전체 모집단 기준으로는 악화된다. | 목표군(n=84) mean MAE −12.8%(23.91→20.86, 50/84 개선) vs. 전체(n=1,698) MAE +8.53%(10.895→11.825); 위드닝 445개 중 true positive 63/false positive 382, FP 그룹 평균 +4.80° 악화(310/382); 5개 history 지표(속도/속도표준편차/CV/가속도/방향전환) 모두 TP-FP 분리 실패(AUC 0.488-0.555, 모두 p>0.15) | `results/presentation_20260816/adaptive_k_negative_result.png`, `results/speculative/consolidated/adaptive_k_degraded_before_after.png`, `adaptive_k_fp_diagnosis_boxplot.png` | **본문** (한계/향후연구 절) — 항목4의 상관관계 주장을 개입 실험으로 검증한 유일한 항목이므로 상관 대 인과를 구분하는 논문 서사에 실질적 가치가 있음; 실패라서 부록으로 격하하지 않음 | **강함(성격이 다름) — 설계 유효성과 트리거 부적절성을 분리해 입증.** 목표군 개선(−12.8%)은 개입 자체가 유효함을 보이고, false positive 분석(5개 지표 모두 무분리)은 "이 정보원으로는 트리거를 만들 수 없다"는 것을 독립적으로 입증 — 하나의 실패 실험에서 두 개의 서로 다른, 둘 다 신뢰할 수 있는 결론을 얻은 사례. Full-scale(n=1,698), 이 인스턴스(2026-08-23)의 latency는 다른 인스턴스와 비교 불가(accuracy만 교차 인스턴스 재현 검증됨) |
...
- **Negative result지만 본문 가치 있음**: #11 (adaptive-K) — #4의
  상관관계 주장을 개입 실험으로 검증한 유일한 항목. 결과가 실패라서
  강도가 낮은 것이 아니라, 두 개의 독립적으로 강한 하위 결론(설계
  유효성 / 트리거 정보 부족)을 담고 있어 성격상 다른 항목들과 나란히
  줄 세울 수 없다 — 별도 카테고리로 취급.

## 발표에만 어울리고 논문에는 부적합한 항목

없음 — 위 11개 전부 논문 어딘가(본문 또는 부록)에 배치 가능하다고 판단.
다만 `results/presentation_20260816/presentation_storyline.md`가 시간
제약상 발표에서는 이 중 5개만(모듈①의 #7/#8, 모듈②의 #6, 심층분석의
#3/#4/#11)만 다룬다 — 발표 선별과 논문 배치는 별개 결정이다.
